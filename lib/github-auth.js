...
const gh_token =
  process.env.GH_TOKEN || (serverSecrets && serverSecrets.gh_token);
...
function githubRequest(request, url, query, cb) {
  ...
  const githubToken = gh_token ? gh_token : getReqRemainingToken();
  ...
  request(url, {headers: headers}, function(err, res, buffer) {
    if (gh_token !== null && githubToken !== null && err === null) {
